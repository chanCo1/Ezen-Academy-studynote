...
const MyEffect = () => {
  ...
  useEffect(() => {
    console.clear();
    console.log(`[MyEffect1]>> [${new Date().toLocaleString()}] ::: 화면에 컴포넌트가 처음 로드될 때 처리되어야 할 기능`);
  ...
  useEffect(() => {
    console.log(`[MyEffect2]>> [${new Date().toLocaleString()}] ::: 화면에 컴포넌트가 처음 로드되거나 state, props 중 하나라도 변경될 경우 호출됨`)
  })

  /** 이 컴포넌트가 화면에 막 등장할 때와 특정 state, props값이 변경될 때만 실행 */
  useEffect(() => {
    console.log(`[MyEffect3]>> [${new Date().toLocaleString()}] ::: myBrightness값이 변경됨`)
  },[myBrightness])

  /** state값이 변경되어 화면이 다시 렌더링되거나 화면 이동 등의 이유로 이 컴포넌트가 사라질 때 실행됨 */
  useEffect(() => {
    return () => {
      console.log(`[MyEffect4]>> [${new Date().toLocaleString()}] ::: 이 컴포넌트가 화면에서 사라지기 직전에 처리되어야 할 기능`)
    }
  })
  ...
  return (
    <div>
      ...
      <div>
        <img alt='react' src={react} width={300} style={{filter: `brightness(${myBrightness}%)`}} />
      </div>
    </div>
  );
};
