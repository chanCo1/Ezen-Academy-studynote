...
// GET 파라미터 추출 기능을 갖는 useLocation() 함수를 react-router-dom 패키지로 부터 참조함.
...
const DepartmentGet = () => {
  ...
  console.log("요청된 메세지 값 = %s (%s)", query.get("msg"), typeof query.get("msg"));
  console.groupEnd();

  // 한 페이지에서 GET파라미터에 따라 다르게 푠현할 데이터 준비
  // 실전에서는 이 값에 해당하는 JSON을 백엔드로 부터 받아와야 한다. => Ajsx
