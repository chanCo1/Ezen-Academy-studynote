...
const GradeList = () => {
  ...
  // TODO: 화면에 표시할 성적표 데이터를 저장하기 위한 상태 변수
  ...
  // TODO: axios-hook에 의해 생성된 상태값인 data가 변경되었을 때(ajax 로딩이 완료되었을 때) 실행될 hook
  ...
  // TODO: 백엔드로부터 데이터 삭제하기 - 자체 캐시기능 방지, 삭제 버튼 이벤트에 의해 호출되어야 하므로 메뉴얼 실행 모드
  const [{loading2}, sendDelete] = useAxios(
    {method: 'DELETE'}, 
    {useCache: false, manual: true}
  );

  // TODO: 삭제 버튼 클릭시 호출될 이벤트 핸들러
  const onDeleteClick = e => {
    ...
    if(window.confirm(`정말 ${name} 학생의 성적을 삭제하시겠습니까?`)) {
      // 백엔드에 삭제 요청 -> 입력, 수정, 삭제는 async-await 문법으로 처리해야한다.
      (async () => {
        let json = null;
